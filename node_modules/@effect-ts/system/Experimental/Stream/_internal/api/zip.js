...
function zip_(...[s1, s2, ...streams]) {
  const init = ZipWith.zipWith_(s1, s2, Tp.tuple);
  return A.reduce_(streams, init, (acc, v) => ZipWith.zipWith_(acc, v, (a, b) => Tp.append_(a, b)));
}
function zip(...[s1, ...streams]) {
  return (self) => zip_(self, s1, ...streams);
}

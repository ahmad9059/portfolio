...
var mapAccumEffect_exports = {};
__export(mapAccumEffect_exports, {
...
function mapAccumEffect_(self, s, f) {
  const accumulator = (s2) => CH.readWith((in_) => CH.unwrap(T.suspend(() => {
    const outputChunk = CK.builder();
    const emit = (a) => T.asUnit(T.succeedWith(() => {
      outputChunk.append(a);
    }));
    return T.fold_(T.reduce_(in_, s2, (s1, a) => T.chain_(f(s1, a), (sa) => T.as_(emit(Tp.get_(sa, 1)), Tp.get_(sa, 0)))), (failure) => {
...
module.exports = __toCommonJS(mapAccumEffect_exports);

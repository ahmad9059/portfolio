var __create = Object.create;
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __getProtoOf = Object.getPrototypeOf;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __markAsModule = (target) => __defProp(target, "__esModule", { value: true });
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __reExport = (target, module2, copyDefault, desc) => {
  if (module2 && typeof module2 === "object" || typeof module2 === "function") {
    for (let key of __getOwnPropNames(module2))
      if (!__hasOwnProp.call(target, key) && (copyDefault || key !== "default"))
        __defProp(target, key, { get: () => module2[key], enumerable: !(desc = __getOwnPropDesc(module2, key)) || desc.enumerable });
  }
  return target;
};
var __toESM = (module2, isNodeMode) => {
  return __reExport(__markAsModule(__defProp(module2 != null ? __create(__getProtoOf(module2)) : {}, "default", !isNodeMode && module2 && module2.__esModule ? { get: () => module2.default, enumerable: true } : { value: module2, enumerable: true })), module2);
};
var __toCommonJS = /* @__PURE__ */ ((cache) => {
  return (module2, temp) => {
    return cache && cache.get(module2) || (temp = __reExport(__markAsModule({}), module2, 1), cache && cache.set(module2, temp), temp);
  };
})(typeof WeakMap !== "undefined" ? /* @__PURE__ */ new WeakMap() : 0);
var foldEffect_exports = {};
__export(foldEffect_exports, {
  foldEffect: () => foldEffect
});
var CK = __toESM(require("../../../Collections/Immutable/Chunk/index.js"));
var Tp = __toESM(require("../../../Collections/Immutable/Tuple/index.js"));
var T = __toESM(require("../../../Effect/index.js"));
var O = __toESM(require("../../../Option/index.js"));
var CH = __toESM(require("../Channel/index.js"));
var C = __toESM(require("./core.js"));
function foldEffect(z, contFn, f) {
  const foldChunkSplitM = (z2, chunk, contFn2, f2) => {
    const fold = (s, chunk2, idx, len) => {
      if (idx === len) {
        return T.succeed(Tp.tuple(s, O.none));
      } else {
        return T.chain_(f2(s, CK.unsafeGet_(chunk2, idx)), (s1) => {
          if (contFn2(s1)) {
            return fold(s1, chunk2, idx + 1, len);
          } else {
            return T.succeed(Tp.tuple(s1, O.some(CK.drop_(chunk2, idx + 1))));
          }
        });
      }
    };
    return fold(z2, chunk, 0, CK.size(chunk));
  };
  const reader = (s) => CH.readWith((in_) => CH.chain_(CH.fromEffect(foldChunkSplitM(s, in_, contFn, f)), ({
    tuple: [nextS, leftovers]
  }) => O.fold_(leftovers, () => reader(nextS), (l) => CH.as_(CH.write(l), nextS))), (err) => CH.fail(err), (_) => CH.end(s));
  return new C.Sink(contFn(z) ? reader(z) : CH.end(z));
}
module.exports = __toCommonJS(foldEffect_exports);
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  foldEffect
});

...
var raceBoth_exports = {};
__export(raceBoth_exports, {
  raceBoth: () => raceBoth,
  raceBoth_: () => raceBoth_
});
...
function raceBoth_(self, that, capacity = 16) {
  const managed = M.map_(M.let_(M.let_(M.let_(M.bind_(M.bind_(M.bind_(M.do, "hub", () => T.toManaged(H.makeBounded(capacity))), "c1", ({
    ...
    reader,
    writer
  }) => CH.mergeWith_(reader, writer, (_) => MH.await_((_2) => T.done(_2)), (done) => MH.done(T.done(done)))), ({
    channel
  }) => new C.Sink(channel));
...
module.exports = __toCommonJS(raceBoth_exports);

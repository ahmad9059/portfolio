...
import { build, compose_, fold, fromRawEffect, fromRawFunction, fromRawFunctionM, fromRawManaged, identity, LayerAllPar, LayerAllSeq, LayerChain, LayerFresh, LayerManaged, LayerMap, LayerSuspend, LayerZipWithPar, LayerZipWithSeq } from "./definitions.js";
...
export function fromValue(has) {
  return resource => new LayerManaged(M.chain_(M.fromEffect(T.succeed(resource)), a => environmentFor(has, a))).setKey(has.key);
}
/**
...
export function fromEffect_(resource, has) {
  return new LayerManaged(M.chain_(M.fromEffect(resource), a => environmentFor(has, a))).setKey(has.key);
}
/**
...
export function fromManaged(has) {
  return resource => new LayerManaged(M.chain_(resource, a => environmentFor(has, a))).setKey(has.key);
}
/**
 * Constructs a layer from a managed resource.
 */
...
export function fromManaged_(resource, has) {
  return new LayerManaged(M.chain_(resource, a => environmentFor(has, a))).setKey(has.key);
}
/**

...
class ChunkInternal {
  ...
  append(a1) {
    ...
    buffer[0] = a1;
    return new AppendN(this, buffer, 1, new import_AtomicNumber.AtomicNumber(1), this.binary && binary);
  }
  prepend(a1) {
    const binary = this.binary && isByte(a1);
    const buffer = this.binary && binary ? alloc(BufferSize) : new Array(BufferSize);
    buffer[BufferSize - 1] = a1;
    return new PrependN(this, buffer, 1, new import_AtomicNumber.AtomicNumber(1), this.binary && binary);
  }
  take(n) {
    ;
    if (n <= 0) {
...
class Empty extends ChunkInternal {
  ...
  get(n) {
    throw new import_GlobalExceptions.ArrayIndexOutOfBoundsException(n);
  }
  materialize() {
    return array_([]);
  }
...
class AppendN extends ChunkInternal {
  ...
  append(a1) {
    ...
    if (this.bufferUsed < this.buffer.length && this.chain.compareAndSet(this.bufferUsed, this.bufferUsed + 1)) {
      ...
      buffer[0] = a1;
      const chunk = array_(this.buffer).take(this.bufferUsed);
      return new AppendN(this.start.concat(chunk), buffer, 1, new import_AtomicNumber.AtomicNumber(1), this.binary && binary);
    }
  }
  copyToArray(n, array) {
...
class PrependN extends ChunkInternal {
  ...
  prepend(a1) {
    ...
    if (this.bufferUsed < this.buffer.length && this.chain.compareAndSet(this.bufferUsed, this.bufferUsed + 1)) {
      ...
      return new PrependN(chunk.concat(this.end), buffer, 1, new import_AtomicNumber.AtomicNumber(1), this.binary && binary);
    }
  }
  [Symbol.iterator]() {
    const k = this.arrayLike();

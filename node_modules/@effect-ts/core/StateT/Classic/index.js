...
function monad(M) {
  return {
    any: () => (s) => M.map((m) => (0, import_Function.tuple)(m, s))(M.any()),
    flatten: (ffa) => (x) => (0, import_DSL.chainF)(M)(([f, us]) => f(us))(ffa(x)),
    map: (f) => (fa) => (x) => M.map(([a, s]) => (0, import_Function.tuple)(f(a), s))(fa(x))
  };
}
